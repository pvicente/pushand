...
# locally and, if it contains a .pushand script, it will run it.
...
if [[ -f "$real_script_new_path" ]]; then
	...
	if [[ ! -w "$real_script_path" ]]; then
		...
		# Make the directory that we back up git-receive-pack in. It has to
		# be in a directory because the script *must* keep the name
		# 'git-receive-pack' (in the latest versions of git).
